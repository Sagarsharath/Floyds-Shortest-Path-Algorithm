...
Floyd.prototype.removeNode = function(event)
{

  
 if(this.size == 2){ 
   alert(" minimum 2 nodes are required for finding shortest paths  "+( this.size-1));
   return -1;
 }

  var e  = window.prompt("Enter the Node No to remove");
  
 if(isNaN(e)||Number(e)>this.size){
   alert("enter a valid  Node from 0 to "+( this.size-1));
   return -1;
 } 

  newindex = Number(e);

this.new_adj_matrix = new Array(this.size-1);

  
   for (i = 0; i < this.size-1; i++)
	{
		this.new_adj_matrix[i] = new Array(this.size-1);
	}
		     ...
		     for (var j = 0; j < this.size-1; j++)
		     	{
			    	...
			    	this.new_adj_matrix[i][j] = this.adj_matrix[newi][newj];
			  
			    }	 
		} 
	this.size = this.size-1;
 this.insertWeight(this.new_adj_matrix); 
 this.setupNew();


}

Floyd.prototype.insertNode = function(event)
{
 
 if(this.size == 8){
    
 alert("max nodes reached");
 	return -1;

 }
    this.size = this.size+1;
    
this.new_adj_matrix = new Array(this.size);

  
   for (i = 0; i < this.size; i++)
	{
		this.new_adj_matrix[i] = new Array(this.size);
	}
	 ...
	 for (i = 0; i < this.size-1; i++)
		{
			for (var j = 0; j < this.size-1; j++)
			{
				this.new_adj_matrix[i][j] = this.adj_matrix[i][j];
...
Floyd.prototype.removeWeight = function(event){

  var e  = window.prompt("Enter the Source Node");
  
 if(isNaN(e)||Number(e)>this.size){
   alert("enter a valid Source Node from 0 to "+( this.size-1));
   return -1;
 }
 
  var f  = window.prompt("Enter the Destination Node");
 
 if(isNaN(f)||Number(f)>this.size){
   alert("enter a valid Destination Node from 0 to "+( this.size-1));
 return -1;
 }

 
 this.adj_matrix[Number(e)][Number(f)]  = -1;

 this.insertWeight(this.adj_matrix); 
 this.setupNew();
...
Floyd.prototype.insertEdgeWeight = function(event)
{
 ...
 return -1;
  ...
  var g  = window.prompt("Enter the Weight");
  
 if(isNaN(g)||Number(g)<1){
   alert("Enter a valid weight");
   return -1;
 }
 //console.log(this.adj_matrix);
 
 this.adj_matrix[Number(e)][Number(f)]  = Number(g);   
...
Floyd.prototype.setupNew = function(){
